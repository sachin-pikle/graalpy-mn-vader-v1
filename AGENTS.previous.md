...
- Keep `micronaut-graalpy` in the app to preserve the Micronaut-native GraalPy story for the session
- Keep `org.graalvm.python:python-embedding` explicit because the code imports and uses the embedding API directly
- Do not add `org.graalvm.polyglot:python-community` for this demo line
- Only move to GraalPy 25.x when the runtime and dependency stack are intentionally upgraded together
...
- Remove duplicate proof paths once the core upload-to-GraalPy path already demonstrates the session point
- Prefer the simplest data handoff that matches the current scope; for text uploads, pass plain text into GraalPy instead of adding encoding layers
- Keep `pom.xml` lean: remove generated Micronaut options, plugins, processors, and dependencies that the current demo does not actively use
- When a class is imported directly in code, prefer keeping that library as an explicit dependency instead of relying on a transitive one
